...
因为目前目前要实现一个校验内核，将登录、鉴权以及其他一些通用操作抽离出来复用。这样一个内核使用起来肯定会搭成分布式结构，部署多个机房多个主机，通过nginx来进行负载均衡。如果不进行session共享，那么session是由tomcat容器管理的，当两次请求分别打进了不同的主机，第二次请求cookie中携带了session，要么session无效重新登录，要么跳过登录使用session时报错，总之体验不好。
